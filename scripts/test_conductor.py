...
def test_parse_args_allows_external_authority_without_reviewers() -> None:
    ...
    assert args.trusted_external_surfaces == ["Cerberus"]


def test_parse_govern_pr_args_allows_external_authority_without_reviewers() -> None:
    args = conductor.parse_args(
        [
            "govern-pr",
            "--repo",
...
def test_ensure_review_source_configured_requires_council_or_external_authority() -> None:
    args = argparse.Namespace(reviewer=[], trusted_external_surfaces=[])

    with pytest.raises(conductor.CmdError, match="configure at least one --reviewer or one --trusted-external-surface"):
        conductor.ensure_review_source_configured(args)
...
def test_show_run_marks_policy_mergeability_pending_before_governance_history(
    tmp_path: pathlib.Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    conn = conductor.open_db(tmp_path / "conductor.db")
    issue = conductor.Issue(number=594, title="pending", body="", url="https://example.com/594", labels=["autopilot"])
    conductor.create_run(conn, "run-594-1", "misty-step/bitterblossom", issue, "claude-sonnet")
    conductor.update_run(
        conn,
        "run-594-1",
        phase="awaiting_governance",
        ...
        builder_sprite="fern",
        pr_number=594,
        pr_url="https://github.com/misty-step/bitterblossom/pull/594",
    )
    ...
    payload = json.loads(capsys.readouterr().out)
    governance = payload["run"]["governance"]
    assert governance["semantic_readiness"]["state"] == "pending"
    assert governance["policy_mergeability"]["state"] == "pending"


def test_show_run_marks_mechanical_mergeability_mergeable_when_ci_passes(
    tmp_path: pathlib.Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    conn = conductor.open_db(tmp_path / "conductor.db")
    issue = conductor.Issue(number=595, title="ci green", body="", url="https://example.com/595", labels=["autopilot"])
    conductor.create_run(conn, "run-595-1", "misty-step/bitterblossom", issue, "claude-sonnet")
    conductor.update_run(
        conn,
        "run-595-1",
        phase="awaiting_governance",
        ...
        builder_sprite="fern",
        pr_number=595,
        pr_url="https://github.com/misty-step/bitterblossom/pull/595",
    )
    wave_id = conductor.start_review_wave(conn, "run-595-1", "review_round", pr_number=595, reviewer_count=1)
    conductor.record_review_artifact(
        conn,
        "run-595-1",
        wave_id,
    ...
    conductor.complete_review_wave(conn, tmp_path / "events.jsonl", "run-595-1", wave_id, "settled")
    conductor.record_event(
        conn,
        tmp_path / "events.jsonl",
        "run-595-1",
        "ci_wait_complete",
        {"passed": True, "output": "all green"},
    )
    ...
    payload = json.loads(capsys.readouterr().out)
    governance = payload["run"]["governance"]
    assert governance["mechanical_mergeability"]["state"] == "mergeable"
    assert governance["mechanical_mergeability"]["reason"] == "required checks passed"


def test_show_run_blocks_policy_mergeability_after_semantic_ready(
    tmp_path: pathlib.Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    conn = conductor.open_db(tmp_path / "conductor.db")
    issue = conductor.Issue(number=596, title="policy block", body="", url="https://example.com/596", labels=["autopilot"])
    conductor.create_run(conn, "run-596-1", "misty-step/bitterblossom", issue, "claude-sonnet")
    conductor.update_run(
        conn,
        "run-596-1",
        phase="blocked",
        ...
        builder_sprite="fern",
        pr_number=596,
        pr_url="https://github.com/misty-step/bitterblossom/pull/596",
    )
    wave_id = conductor.start_review_wave(conn, "run-596-1", "review_round", pr_number=596, reviewer_count=1)
    conductor.record_review_artifact(
        conn,
        "run-596-1",
        wave_id,
    ...
    conductor.complete_review_wave(conn, tmp_path / "events.jsonl", "run-596-1", wave_id, "settled")
    conductor.record_event(
        conn,
        tmp_path / "events.jsonl",
        "run-596-1",
    ...
    governance = payload["run"]["governance"]
    assert governance["semantic_readiness"]["state"] == "ready"
    assert governance["policy_mergeability"]["state"] == "blocked"
    assert governance["policy_mergeability"]["reason"] == conductor.summarize_blocking_reason(
        "pr_feedback_blocked", {"reason": "unchanged_after_revision"}
    )


def test_show_runs_handles_malformed_finding_raw_json_in_batch_loader(
    tmp_path: pathlib.Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    conn = conductor.open_db(tmp_path / "conductor.db")
    issue = conductor.Issue(number=597, title="malformed raw", body="", url="https://example.com/597", labels=["autopilot"])
    conductor.create_run(conn, "run-597-1", "misty-step/bitterblossom", issue, "claude-sonnet")
    conductor.update_run(
        conn,
        "run-597-1",
        phase="blocked",
        ...
        builder_sprite="fern",
        pr_number=597,
        pr_url="https://github.com/misty-step/bitterblossom/pull/597",
    )
    wave_id = conductor.start_review_wave(conn, "run-597-1", "review_round", pr_number=597, reviewer_count=1)
    conn.execute(
        """
        insert into review_findings (
            run_id, wave_id, reviewer, source_kind, source_id, fingerprint, classification,
    ...
    assert rc == 0
    payload = json.loads(capsys.readouterr().out.strip())
    assert payload["run_id"] == "run-597-1"
    assert payload["governance"]["semantic_readiness"]["state"] == "blocked"
...
def _make_run_once_args(
    ...
    *,
    ...
    return argparse.Namespace(
        ...
        worker=["noble-blue-serpent"],
        builder_template=str(pathlib.Path("scripts/prompts/conductor-builder-template.md")),
        reviewer=reviewers if reviewers is not None else ["fern", "sage", "thorn"],
        reviewer_template=str(pathlib.Path("scripts/prompts/conductor-reviewer-template.md")),
        builder_timeout=10,
        review_timeout=10,
        ci_timeout=10,
...
def test_govern_pr_uses_external_authority_without_internal_reviewers(
    ...
    monkeypatch.setattr(
        conductor,
        "merge_pr",
        lambda _r, _repo, pr_num: (events.append(("merge", pr_num)), merge_calls.append(pr_num)),
    )

    rc = conductor.govern_pr(
        _make_govern_pr_args(
            tmp_path,
            issue_number=494,
            pr_number=490,
            run_id="run-479-1",
            reviewers=[],
            trusted_external_surfaces=["Cerberus"],
        )
    )

    assert rc == 0
    assert events.count(("external_wait", 490)) >= 1
    assert events.count(("surfaces", ("Cerberus",))) >= 1
...
def test_run_once_uses_external_authority_without_internal_reviewers(
    monkeypatch: pytest.MonkeyPatch, tmp_path: pathlib.Path
) -> None:
    issue = conductor.Issue(number=494, title="external authority", body="", url="https://example.com/494", labels=["autopilot"])
    builder = conductor.BuilderResult(
        status="ready_for_review",
        branch="factory/494-gov-1",
        pr_number=495,
    ...
    monkeypatch.setattr(
        conductor,
        "ensure_reviewers_ready",
        lambda *_a, **_kw: (_ for _ in ()).throw(AssertionError("internal reviewers should be skipped")),
    )
    monkeypatch.setattr(conductor, "run_builder", lambda *_a, **_kw: (builder, {"status": "ready_for_review"}))
    monkeypatch.setattr(
        conductor,
    ...
    monkeypatch.setattr(conductor, "ensure_required_checks_present", lambda *_a, **_kw: None)
    monkeypatch.setattr(conductor, "list_unresolved_review_threads", lambda *_a, **_kw: [])
    monkeypatch.setattr(conductor, "wait_for_external_reviews", lambda *_a, **_kw: (True, "Cerberus: SUCCESS"))
    monkeypatch.setattr(conductor, "merge_pr", lambda _r, _repo, pr_num: merge_calls.append(pr_num))
    monkeypatch.setattr(conductor, "comment_pr", lambda *_a, **_kw: None)
    monkeypatch.setattr(conductor, "comment_issue", lambda *_a, **_kw: None)
    ...
    assert rc == 0
    assert merge_calls == [495]
    conn = conductor.open_db(pathlib.Path(args.db))
    run_row = conn.execute("select run_id from runs where issue_number = ?", (494,)).fetchone()
    assert run_row is not None
