...
func TestBuildOneShotScriptCleansStatusFiles(t *testing.T) {
	...
	workspace := "/home/sprite/workspace"
	script := buildOneShotScript(workspace, "/home/sprite/workspace/prompt.md", "/home/sprite/workspace/logs/oneshot.log")

	// Must contain cleanup generated by signals package
	wantFiles := []string{"TASK_COMPLETE", "TASK_COMPLETE.md", "BLOCKED.md", "BLOCKED"}
	for _, f := range wantFiles {
		if !strings.Contains(script, f) {
			t.Errorf("buildOneShotScript missing cleanup of %s", f)
		}
	}
	...
	for i, line := range lines {
		if strings.Contains(line, "rm -f") && strings.Contains(line, "TASK_COMPLETE") {
			cleanupIdx = i
		}
