...
fn parse_verdict_skips_braces_in_preamble_prose() {
    ...
    // its JSON block; the parser must find the real document.
    let raw = r#"The `eprintln!("env: {env:?}")` call leaks secrets.

```json
{"verdict": "blocking", "findings": [{"severity": "blocking", "claim": "secrets leak"}]}
```"#;
    let doc = submit::parse_verdict("security", raw).unwrap();
    assert_eq!(doc.verdict, "blocking");
    assert_eq!(doc.findings[0].claim, "secrets leak");
}
