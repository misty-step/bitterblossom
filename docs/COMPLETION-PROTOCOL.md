...
Signal files that agents write to communicate task status back to dispatch and conductor checks.
...
1. **Pre-dispatch cleanup** (`cmd/bb/dispatch.go`): All signal files are removed before dispatch starts to prevent false-positive detection from previous runs.

2. **Agent execution**: The agent works on the task. When done, it writes `TASK_COMPLETE` (success) or `BLOCKED.md` (stuck).

3. **Detection** (`cmd/bb/dispatch.go` and `cmd/bb/dispatch_checks.go`): dispatch checks for `TASK_COMPLETE` / `TASK_COMPLETE.md` and `BLOCKED.md` after the agent run and during recovery polling.
...
The commit check is scoped to the current dispatch by capturing HEAD SHA before the agent run starts. This prevents stale commits from a prior failed dispatch from triggering false successes. When SHA capture fails, the check falls back to comparing HEAD against `origin/master`/`origin/main` with a warning.
...
| Dispatch execution | Completion + blocked checks during agent execution and recovery | `cmd/bb/dispatch.go`, `cmd/bb/dispatch_checks.go` |
| Dispatch cleanup | Removes stale signals before the agent run via the shared contract helpers | `cmd/bb/dispatch.go` |
