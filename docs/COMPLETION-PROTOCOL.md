...
Signal files that agents write to communicate task status back to the transport-managed agent session.
...
1. **Pre-dispatch cleanup** (`cmd/bb/dispatch.go`): All signal files are removed before execution starts to prevent false-positive detection from previous runs.

2. **Agent execution**: The agent works on the task. When done, it writes `TASK_COMPLETE` (success) or `BLOCKED.md` (stuck).

3. **Detection** (`cmd/bb/dispatch.go`): The inline agent session checks for signal files between iterations. `TASK_COMPLETE` or `TASK_COMPLETE.md` → exit 0. `BLOCKED.md` → exit 2.
...
| Agent session | Completion + blocked between iterations | `cmd/bb/dispatch.go` |
| Dispatch cleanup | Removes stale signals before execution start via the shared contract helpers | `cmd/bb/dispatch.go` |
