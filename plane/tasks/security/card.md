...
If `RUN.json.task` is a model-evaluation variant such as `security-kimi` or
`security-glm`, keep the same security lens and output schema. The plane records
...
Injection, secret leakage, authz gaps, unsafe input handling, paths
that move credentials into argv/logs/persistence. IGNORE: theoretical
issues with no reachable path, style, correctness bugs with no security
consequence.
