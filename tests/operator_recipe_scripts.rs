...
if cmd == ["preflight", "--storm", "--json"]:
    ...
    if os.environ.get("BB_FAKE_OPEN_DLQ"):
...
fn submit_storm_recipe_blocks_open_dlq_before_submission_mutation() {
    ...
    assert!(!log.contains("submit"), "open DLQ must block submit:\n{log}");
    assert!(!log.contains("\"run\""), "open DLQ must block member fanout:\n{log}");
    assert!(!log.contains("gate"), "open DLQ must block gate mutation:\n{log}");
    assert!(!log.contains("ack") && !log.contains("replay"), "readiness must not mutate DLQ:\n{log}");
}
...
fn submit_storm_recipe_allows_replayed_and_acknowledged_history() {
    ...
    let log = fs::read_to_string(dir.path().join("bb.log")).unwrap();
    assert!(log.contains("submit"), "resolved history must not block submit: {log}");
    assert!(log.contains("\"run\""), "resolved history must allow member dispatch: {log}");
}
...
fn local_primary_installer_stages_release_atomically_and_requires_explicit_legacy_cleanup() {
    ...
    assert_eq!(fs::read_to_string(install_dir.join("bb")).unwrap(), "release-v1");
    let rendered = fs::read_to_string(home.join("Library/LaunchAgents/com.misty-step.bb-serve.plist")).unwrap();
    ...
    assert!(legacy.exists(), "default install must not silently delete legacy label");
    assert!(String::from_utf8_lossy(&detected.stderr).contains("--retire-legacy-dashboard"));

    fs::remove_file(&release).unwrap();
    fs::create_dir(&release).unwrap();
    let failed = run(&[]);
    assert!(!failed.status.success(), "copying a directory must fail before replacement");
    assert_eq!(fs::read_to_string(install_dir.join("bb")).unwrap(), "release-v1");
    ...
    assert!(!legacy.exists(), "explicit cleanup must remove retired plist");
