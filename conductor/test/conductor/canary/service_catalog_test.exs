defmodule Conductor.Canary.ServiceCatalogTest do
  ...
  test "loads valid services with defaults", %{path: path} do
    ...
    assert linejam.clone_url == "https://git.example.com/misty-step/linejam.git"
  ...
  test "rejects invalid clone_url values", %{path: path} do
    File.write!(
      path,
      """
      [[service]]
    ...
    assert {:error, msg} = ServiceCatalog.load(path)
    assert msg =~ "missing required 'clone_url'"
  end

  test "requires clone_url explicitly", %{path: path} do
    File.write!(
      path,
      """
      [[service]]
  ...
  test "rejects invalid default_branch values", %{path: path} do
    File.write!(
      path,
      """
      [[service]]
    ...
    assert {:error, msg} = ServiceCatalog.load(path)
    assert msg =~ "invalid default_branch"
  end
