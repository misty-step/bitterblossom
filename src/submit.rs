...
pub fn parse_verdict(kind: &str, raw: &str) -> Result<VerdictDoc> {
    let start = raw.find('{').context("no JSON object in verdict output")?;
    let end = raw
        .rfind('}')
        .context("no closing brace in verdict output")?;
    let mut doc: VerdictDoc =
        serde_json::from_str(&raw[start..=end]).context("verdict JSON malformed")?;
