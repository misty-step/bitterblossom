defmodule Conductor.ApplicationTest do
  ...
  defmodule SuccessfulDashboardEndpoint do
  ...
  test "start_dashboard returns an error when the endpoint secret is missing" do
    original_endpoint = Application.get_env(:conductor, :dashboard_endpoint_module)
    original_start_dashboard = Application.get_env(:conductor, :start_dashboard)
    original_endpoint_config = Application.get_env(:conductor, Conductor.Web.Endpoint)
    ...
    Application.put_env(:conductor, :dashboard_endpoint_module, SuccessfulDashboardEndpoint)
    Application.put_env(:conductor, :start_dashboard, true)
    ...
    assert {:error, :missing_dashboard_secret_key_base} =
             Conductor.Application.start_dashboard()
  end

  test "start_dashboard returns the overridden port and persists it in endpoint config" do
    original_endpoint = Application.get_env(:conductor, :dashboard_endpoint_module)
    original_start_dashboard = Application.get_env(:conductor, :start_dashboard)
    original_endpoint_config = Application.get_env(:conductor, Conductor.Web.Endpoint)
    ...
    assert {:ok, 4444} = Conductor.Application.start_dashboard(port: 4444)
    assert Conductor.Application.dashboard_port() == 4444

    assert Application.get_env(:conductor, Conductor.Web.Endpoint)
           |> Keyword.get(:http, [])
           |> Keyword.get(:port) == 4444
  end
end
