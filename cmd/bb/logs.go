...
func runLogs(ctx context.Context, stdout, stderr io.Writer, spriteName string, follow bool, lines int, jsonMode bool) error {
	...
	logPath := workspaceAgentLogPath(workspace)
...
func writeLogsNoTaskMsg(stderr io.Writer, spriteName string) error {
	_, err := fmt.Fprintf(stderr,
		"No active task on %q.\nThe sprite is reachable, but no agent is running and agent.log is empty.\nTry: bb status %s\n",
		spriteName,
		spriteName,
	)
	return err
...
func spriteHasRunningAgent(ctx context.Context, s *sprites.Sprite) bool {
	check := `pgrep -f '[b]b-agent-session|[c]laude|[c]odex' >/dev/null 2>&1`
