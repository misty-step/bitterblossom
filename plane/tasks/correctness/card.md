# Correctness verdict commission
...
If `RUN.json.task` is a model-evaluation variant such as `correctness-kimi` or
`correctness-glm`, keep the same correctness lens and output schema. The plane
records those variants under eval-only verdict kinds; they are not canonical
gate members.
...
Logic errors, broken invariants, unhandled failure paths, concurrency
bugs, data races, off-by-ones. IGNORE: style, naming, formatting, docs,
security (another member owns it), product judgment.
