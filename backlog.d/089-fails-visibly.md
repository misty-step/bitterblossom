...
- [x] Each run state and attempt phase has a documented freshness contract:
...
- [x] Serve-mode dispatch writes meaningful progress while execute is active or
...
- [x] An attention-debt brake aggregates open DLQ, parked tasks, stale runs, and
...
- [x] `bb status --json`, `/api/status`, and `bb runs show --json` expose the
      freshness/outbox/escalation state needed by humans and agents.
- [x] `./scripts/verify.sh` passes, including fixture drills for stale executing,
      stuck gate member, notify retry, and attention-debt admission refusal.

## Children

- [x] Freshness-contract table in docs plus status JSON fields for every
      non-terminal state.
...
- [x] Attention-debt brake for reflex ingress.
- [x] Self-drill chaos reflex task, report artifact, and failure notification.
- [x] Backlog consolidation notes for 051, 083, 085, 087, and any storm-timeout
...
2026-07-01 implementation consolidation:

- PR #879: serve watchdog escalates stale executing/recovery through outbox.
- PR #880: submission arm timeout plus quorum policy; gate policy visible in
  status.
- PR #881: weekly self-drill task and deterministic chaos fixture prove stale
  arm -> outbox -> notification.
