...
- [x] `bb mcp serve` starts a stdio MCP server with no network listener by default.
- [x] Tool list includes read-only tools: `bb_status`, `bb_check`, `bb_tasks`, `bb_runs_list`, `bb_runs_show`, `bb_dlq_list`, `bb_preflight`, `bb_gate`, and artifact read/list if backlog 079 lands first.
- [x] Tool outputs are generated by shared view helpers also used by CLI/API; no MCP-only bespoke status/run shapes.
- [x] The server has a read-only default mode and rejects mutating calls in that mode.
- [x] MCP tests prove `tools/list` and focused `tools/call` requests against `examples/local-plane`.
- [x] `skills/bitterblossom/` documents MCP-first, CLI-fallback routing.
...
### 2026-07-02 artifact MCP closure slice

- Added the final read-only artifact tools: `bb_artifacts_list` and
  `bb_artifact_read`.
- The tools call the same artifact helpers as CLI; `tests/mcp_cli.rs` compares
  their output directly against `bb artifacts list/read --json` for the same
  run and confirms unsafe paths return MCP `isError: true`.
- Updated the portable skill and spine docs with MCP-first artifact routing.
- No mutating MCP tools exist. Unknown/would-be mutating calls still fail as
  JSON-RPC `-32602`.
- With artifact list/read present, backlog 078's read-only MCP layer is closed.
