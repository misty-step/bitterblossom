...
The `cmd` directory serves as the entry point for Bitterblossom's command-line tools, primarily housing the `bb` utility. `bb` bridges local user input and remote execution by managing sprite setup, prompt dispatch, log streaming, status inspection, and process recovery.
...
- **`dispatch.go`**: The primary orchestration engine. It handles pre-flight connectivity checks, synchronizes GitHub repositories to the remote environment, renders prompt templates, and starts an inline agent session. It maintains a cancellation context to terminate remote tasks if safety thresholds are breached.
