...
def prepare_run_workspace(runner: Runner, sprite: str, repo: str, run_id: str, lane: str) -> str:
    ...
    script = "\n".join(
        [
            ...
            'if git -C "$mirror" show-ref --verify --quiet refs/heads/master; then',
            '  base_ref="master"',
            'elif git -C "$mirror" show-ref --verify --quiet refs/heads/main; then',
            '  base_ref="main"',
            "else",
            '  base_ref=$(git -C "$mirror" symbolic-ref --short HEAD)',
            "fi",
