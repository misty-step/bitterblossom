...
func newDispatchCmdWithDeps(deps dispatchDeps) *cobra.Command {
	...
	command.Flags().StringVar(&opts.App, "app", opts.App, "Fly app name")
	command.Flags().StringVar(&opts.Token, "token", opts.Token, "Fly token (or FLY_API_TOKEN/FLY_TOKEN)")
	command.Flags().StringVar(&opts.APIURL, "api-url", opts.APIURL, "Fly Machines API base URL")
