...
*   **Execution Layer (Claude Code):** Agentic intelligence, task decomposition, and repository analysis are delegated to Claude Code skills.
...
*   **[`cmd/bb/dispatch.go`](../cmd/bb/dispatch.go)**: Owns the inline agent-session loop that invokes the harness, manages signal files, and enforces execution limits.

### Dependencies and Technical Constraints
*   **Harness:** Claude Code pinned to the Sonnet 4.6 runtime.
...
*   **Timeout Limitations:** The Go-based transport is strictly required because the Claude Code Bash tool imposes a 600-second timeout, which is insufficient for the 30+ minute durations typical of full agent sessions.
