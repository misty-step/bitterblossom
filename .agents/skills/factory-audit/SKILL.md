...
- If a run fails after builder work, inspect `.bb/conductor/<run_id>/builder-result.json` and the PR surface before trusting the failure comment or `show-runs`.
- Confirm terminal lease behavior too: failed or completed runs should not leave a live non-blocked lease behind.
